...
# Full Educational DES Implementation With All Tables
# Author: ChatGPT
...
def des_encrypt_block(plaintext64, key64):
    ...
    for i in range(16):
        newR = xor(L, feistel(R, round_keys[i]))
        L = R
        R = newR
...
def text_to_bits(s):
    return [int(bit) for char in s.encode() for bit in f"{char:08b}"]

def bits_to_text(bits):
    chars = []
    for b in range(0, len(bits), 8):
        chars.append(chr(int("".join(str(bit) for bit in bits[b:b+8]), 2)))
    return "".join(chars)


# ------------------------------ MAIN ------------------------------
if __name__ == "__main__":
    plaintext = input("Enter 8‑byte plaintext: ")
    key = input("Enter 8‑byte key: ")

    plaintext_bits = text_to_bits(plaintext)
    key_bits = text_to_bits(key)

    ciphertext = des_encrypt_block(plaintext_bits, key_bits)
    decrypted = des_decrypt_block(ciphertext, key_bits)

    print("\nCiphertext (bits):", "".join(map(str, ciphertext)))
    print("Decrypted Text:", bits_to_text(decrypted))
