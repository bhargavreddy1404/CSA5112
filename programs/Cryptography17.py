...
PC2 = [
    ...
    39,56,34,53,46,42,50,36, 29,32,25,54,35, 38,43
]
...
def sbox_substitution(bits):
    ...
    for i in range(8):
        ...
        result += format(s_val, '04b')
    return result

def feistel(right, key):
    expanded = permute(right, E_BOX)
    x = xor(expanded, key)
    s_output = sbox_substitution(x)
...
def des_decrypt(cipher64, key64):
    ...
    keys = keys[::-1]             # REVERSE ORDER FOR DECRYPTION

    block = permute(cipher64, IP)
...
print("\nDecrypted plaintext (64-bit binary):")
print(plain)
